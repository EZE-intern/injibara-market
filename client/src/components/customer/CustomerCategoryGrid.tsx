...
const getCategoryIconNode = (slugOrName: string): ReactNode => {
  const clean = slugOrName.toLowerCase().trim().replace(/\s+/g, "-");
  
  if (clean.includes("elect")) return <Tv className="h-6 w-6 text-brand-600" />;
  if (clean.includes("vehic") || clean.includes("bajaj") || clean.includes("car")) return <Car className="h-6 w-6 text-brand-600" />;
  if (clean.includes("prop") || clean.includes("land") || clean.includes("build")) return <Home className="h-6 w-6 text-brand-600" />;
  if (clean.includes("fash") || clean.includes("cloth")) return <Shirt className="h-6 w-6 text-brand-600" />;
  if (clean.includes("agri") || clean.includes("teff") || clean.includes("crop")) return <Wheat className="h-6 w-6 text-brand-600" />;
  if (clean.includes("live") || clean.includes("cow") || clean.includes("sheep") || clean.includes("ox")) return <Beef className="h-6 w-6 text-brand-600" />;
  if (clean.includes("furn") || clean.includes("home") || clean.includes("garden")) return <Armchair className="h-6 w-6 text-brand-600" />;
  if (clean.includes("food") || clean.includes("drink") || clean.includes("bever") || clean.includes("coffee")) return <Coffee className="h-6 w-6 text-brand-600" />;
      ...
      <div className="mx-auto max-w-7xl px-6 md:px-12 lg:px-16">
        ...
        <div className="flex items-center justify-between mb-8">
          <div>
            ...
            <p className="text-xs text-gray-500 mt-1">Browse active listings across Injibara</p>
          </div>
          <Link
            to="/products"
        ...
        <div className="grid grid-cols-2 gap-4 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6">
          {categories.map((category) => (
            <Link
              ...
              className="group flex flex-col items-center justify-center rounded-xl border border-gray-200 bg-white p-5 text-center transition hover:border-brand-600 hover:shadow-md"
            >
              {/* Icon Container */}
              ...
              {/* Name */}
              <h3 className="mt-3 text-sm font-semibold text-gray-800 transition-colors group-hover:text-brand-700 truncate max-w-full">
                {category.name}
              </h3>
            </Link>
          ))}
